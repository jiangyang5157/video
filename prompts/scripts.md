...
### 3. 开场与节奏控制 (Hook & Cadence Control)

* **开场钩 (Opening Hook, 前 15s)**：开场即抛一个能勾住观众的悬念/冲突/情绪问号（形态随 Preset：A=认知冲突/生死抉择、B=关系张力/离奇误会、C=奇观/即刻威胁），随后再平滑引入设定。
...
* **（可选·强留存模式）单集情感公式与结尾钩**：仅当用户目标是竖屏投流/强完播短剧或系列（明示要求上瘾感/完播）时启用——按 爽/甜/虐/惊 之一组织"期待→延迟→兑现"循环；**每集末镜以悬念或反转收尾（结尾钩）**；反转密度服从上面的节奏钩子。文艺/治愈/纪实等不追求强留存的，**不启用本条**，避免强塞冲突。
...
* **（可选增强）色彩脚本**：需要跨场情绪对比/长线时，可自愿在封面头后补一行"每幕主导色与情绪"（如 现实=冷青 / 梦境=暖白），作为色彩叙事锚；非必填。
