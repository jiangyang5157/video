...
剧本每幕仅保证 6-8 个事件，不足以填满按"每镜 4-5 秒"推导的该幕镜头数（默认 5 幕时约 8-12 个/幕）。以下镜头由你**主动补造**（按该幕镜头缺口决定数量，默认每幕 1-3 个），不视为偷工减料：
...
* **@Char_Main**：[如 "指节不自觉地按紧枪套扣 / 无意识触碰左耳耳机 / 沉默时眼球微移"]
* **@Char_Target**：[如 "无名指推眼镜中轴 / 喉结上下吞咽 / 抿唇后下巴收紧"]
* **@Char_Pivot**：[系统级动作，如 "界面微光脉冲 / 合成音单调复述"]
...
2. **[Video Motion Prompt] (图生视频/文生视频)**：
   ...
   * **运镜动词标准化（严禁模糊词汇）**：摄影机轨迹**必须**优先使用以下标准专业词汇，防止视频模型方向混淆：
...
> **时长与颗粒度硬性指标**：以剧本头部声明的【目标时长 / 幕数 / 幕段时间码】为准（未声明时默认 4-5 分钟 / 5 幕）。镜头颗粒度固定为 **每镜 4-5 秒**，故全片镜头数 ≈ 目标秒数 ÷ 4~5（默认 4-5 分钟 → **40-60 个 Shot**）；每个幕段的镜头数 ≈ 该幕秒数 ÷ 4~5（默认每幕 **8-12 个**），编号按实际幕数连续（Shot 1.x, 2.x, …, N.x），严禁偷工减料，也严禁为凑 40-60 而超出目标时长。
>
> **时间线闭合（尽量）**：每幕镜头合计时长对齐剧本声明的该幕时间码边界（默认 5 幕时约 55-60 秒/幕）；全片镜头合计尽量达到目标时长。缺口明显时，优先在动作密集幕段补环境 / 过渡镜头，而非拉长单镜头时长。

### Shot [X.X]：[镜头简述/戏剧功能]
* **推荐时长**：[3s / 5s]
* **景别与视角**：[极特写 / 特写 / 中景 / 全景 / 高空俯瞰 / 90度侧影 / 正反打特写]
* **中文物理描述**：[从角色微动作库取用的潜台词动作 + 光影材质 + 摄影机轨线描述]
* **Foley 物理音效**：[精准微观音效，如 Audio of rain hitting black trench coat, clock ticking]

**1. 静态首帧 Prompt (Flow / 生图工具)**:
```text
@Char_Main, [Time & Key Light Anchor], [Clothing & Materials], [Facial Expression], [Lighting & Atmosphere], cinematic photorealistic, 8k resolution, Arri Alexa LF capture, --ar 16:9
```
